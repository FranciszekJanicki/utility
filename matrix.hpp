...
namespace utility {
    ...
    struct Matrix {
        ...
        static Data make_data(Init init) noexcept
        {
            Data result;
        ...
        Matrix(Init init) noexcept : data{make_data(init)}
        {}
        ...
        Row const& operator[](this Matrix const& self, std::size_t row)
        {
            if (row > N) {
        ...
        Row& operator[](this Matrix& self, std::size_t row)
        {
            if (row > N) {
                throw std::runtime_error{"Out of bounds"};
            }
        ...
        T const& operator[](this Matrix const& self, std::size_t row, std::size_t col)
        {
            if (row > N || col > M) {
                throw std::runtime_error{"Out of bounds"};
            }
        ...
        T& operator[](this Matrix& self, std::size_t row, std::size_t col)
        {
            if (row > N || col > M) {
                throw std::runtime_error{"Out of bounds"};
            }
        ...
        Matrix& operator=(this Matrix& self, Init init)
        {
        ...
        Matrix& operator*=(this Matrix& self, T scale) noexcept
        {
            self = matrix_scale(self, scale);
            return self;
        }

        Matrix& operator/=(this Matrix& self, T scale)
        {
        ...
        Matrix& operator^=(this Matrix& self, T power)
        {
    ...
    template <typename T, std::size_t N, std::size_t M>
    Matrix<T, N, M> matrix_scale(Matrix<T, N, M> const& matrix, T scale) noexcept
    {
        Matrix<T, N, M> result;
        for (std::size_t i{}; i < N; ++i) {
            for (std::size_t j{}; j < M; ++j) {
    ...
    template <typename T, std::size_t N>
    Matrix<T, N, N> matrix_power(Matrix<T, N, N> const& matrix, T power) noexcept
    {
        if (power == 1) {
            return matrix;
        }
    ...
    template <typename T, std::size_t N, std::size_t M>
    Matrix<T, N, M> operator*(T scale, Matrix<T, N, M> const& matrix) noexcept
    {
        return matrix_scale(matrix, scale);
    }

    template <typename T, std::size_t N, std::size_t M>
    Matrix<T, N, M> operator*(Matrix<T, N, M> const& matrix, T scale) noexcept
    {
        return matrix_scale(matrix, scale);
    }

    template <typename T, std::size_t N, std::size_t M>
    Matrix<T, N, M> operator/(Matrix<T, N, M> const& matrix, T scale)
    {
        if (scale == static_cast<T>(0)) {
            throw std::runtime_error{"Division by 0!"};
        }
    ...
    template <typename T, std::size_t N>
    Matrix<T, N, N> operator^(Matrix<T, N, N> const& matrix, T power)
    {
