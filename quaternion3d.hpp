#ifndef UTILITY_QUATERNION3D_HPP
#define UTILITY_QUATERNION3D_HPP
...
namespace utility {
    ...
    struct Quaternion3D {
        ...
        {
            auto im{static_cast<T>(1) / self.magnitude()};
        ...
        Quaternion3D& operator*=(this Quaternion3D& self, T factor)
        {
            self.w *= factor;
            self.x *= factor;
            self.y *= factor;
    ...
    template <typename T>
    Quaternion3D<T> operator*(Quaternion3D<T> const& quaternion, T factor)
    {
    ...
    template <typename T>
    Quaternion3D<T> operator*(T factor, Quaternion3D<T> const& quaternion)
    {
...
#endif // UTILITY_QUATERNION3D_HPP