#ifndef UTILITY_ROTATION3D_HPP
#define UTILITY_ROTATION3D_HPP
...
namespace utility {
    ...
    struct Rotation3D {
        ...
        Rotation3D& operator*=(this Rotation3D& self, T factor) noexcept
        {
            self.x *= factor;
            self.y *= factor;
            self.z *= factor;
    ...
    template <typename T>
    Rotation3D<T> operator*(T factor, Rotation3D<T> const& matrix) noexcept
    {
        return Rotation3D<T>{matrix.x * factor, matrix.y * factor, matrix.z * factor};
    }

    template <typename T>
    Rotation3D<T> operator*(Rotation3D<T> const& matrix, T factor) noexcept
    {
        return Rotation3D<T>{matrix.x * factor, matrix.y * factor, matrix.z * factor};
    }
