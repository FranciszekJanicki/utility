...
namespace utility {
    ...
    struct Vector3D {
        ...
        Vector3D& operator*=(this Vector3D& self, T factor)
        {
            self.x *= factor;
            self.y *= factor;
            self.z *= factor;
    ...
    template <typename T>
    Vector3D<T> operator*(T factor, Vector3D<T> const& vector)
    {
        return Vector3D<T>{vector.x * factor, vector.y * factor, vector.z * factor};
    }

    template <typename T>
    Vector3D<T> operator*(Vector3D<T> const& vector, T factor)
    {
